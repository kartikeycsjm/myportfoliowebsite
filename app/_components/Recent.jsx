...
const Recent = ({ type, name, text, image, ltext, llink, all }) => {
    return (
        ...
        md:w-[500px]
        sm:w-[500px]'>
            <h3 className='text-[20px] font-bold pl-5'>{name}</h3>
            <Image src={image} className='pl-5 px-4 w-[90px]' />
            <p className='pl-5 text-justify'>{text}</p>
            <Link className='pl-5 text-blue-800 underline underline-offset-1' href={`${llink}`}>{ltext}</Link>
